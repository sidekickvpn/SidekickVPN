...
from char_type_detector import *
SRC_IP = "0.0.0.0"
GOOGLE_KEYSTROKE_SIZE = 46
MIN_DELAY = 0.15
MAX_DELAY = 2.5
...
#my_detector = CharTypeDetector(SRC_IP, GOOGLE_KEYSTROKE_SIZE, MIN_DELAY, MAX_DELAY)
google_detectors = []
for i in range(256):
	google_detectors.append(CharTypeDetector(SRC_IP, GOOGLE_KEYSTROKE_SIZE, MIN_DELAY, MAX_DELAY))
...
def check_pkt(pkt):
  ...
  if pkt.haslayer(IP):
    ...
    ret = google_detectors[detector_key].add_packet(pkt)
    if bool(ret):
      if ret > 2:
        sendReport({
          "name": "Report - Google Auto-complete",
          "severity": "HIGH",
          "message": "A search query of {} characters was executed on Google.".format(ret),
          "vpnIp": vpnIP
        })
